...
//开始 暂停
function progressTimeControl(img,imagePlay) {
  debugger;
  if ($(img).attr("title") == "暂停") {
        $(img).attr("title", "开始");
        $(img).css("background-image", "url(static/images/play.png)");
        window.clearInterval(_mProgressTimer);

    }else {
    imgpicker.play();
    $(img).attr("title", "暂停");
        $(img).css("background-image", "url(static/images/pause.png)");
...
function progressTimeStop() {
    ...
    $("#progressTime_control").css("background-image", "url(static/images/play.png)");
    $("#scroll_Thumb").css("margin-left", "0px");
    $("#scroll_Track").css("width", "0px");
    ScrollBar.value = 0;
    _index = 0;
